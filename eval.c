/* Last modified Time-stamp: <2015-06-20 19:50:26 s>
 * @(#)eval.c
 */
...
char is_and(object *exp) {
    return is_tagged_list(exp, and_symbol) || is_tagged_list(exp, and_symbol1);
}
...
char is_or(object *exp) {
    return is_tagged_list(exp, or_symbol) || is_tagged_list(exp, or_symbol1);
}
